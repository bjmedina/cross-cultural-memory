...
%       .AB_vs_AC : p-value for r(A,B) != r(A,C)  [does group B vs C matter for A?]
%       .AB_vs_BC : p-value for r(A,B) != r(B,C)  [does group A vs C matter for B?]
%       .AC_vs_BC : p-value for r(A,C) != r(B,C)  [does group A vs B matter for C?]
%       .pmat     : 3x3 symmetric matrix (order: AC, AB, BC matching plot order)
%       .diffs    : struct with bootstrap difference distributions
%       .ci       : struct with 95% CIs for each difference
%
%   Bryan Medina ? Feb 2026 (statistical fix)
    ...
    valid = ~isnan(meanA) & ~isnan(meanB) & ~isnan(meanC);
    ...
    % ---------- compute differences ----------
    % The key: within each bootstrap iteration, all 3 correlations use
    % the SAME resampled data, so the differences are properly paired.
    ...
    % Bar order in plots: [AC, AB, BC] (indices 1, 2, 3)
    % So pmat(1,2) = p for AC vs AB = p_AB_AC (same comparison)
    % pmat(1,3) = p for AC vs BC = p_AC_BC
    % pmat(2,3) = p for AB vs BC = p_AB_BC
    pmat = zeros(3);
    pmat(1,2) = p_AB_AC; pmat(2,1) = p_AB_AC;  % AC vs AB
    pmat(1,3) = p_AC_BC; pmat(3,1) = p_AC_BC;  % AC vs BC
    pmat(2,3) = p_AB_BC; pmat(3,2) = p_AB_BC;  % AB vs BC
    ...
    if verbose
        ...
        fprintf('\n=== Paired Bootstrap Comparison (%s, %s-level, %s, nBoot=%d) ===\n', ...
            upper(trialType), dimLabel, method, nBoot);
        fprintf('Observed: r(A,B)=%.3f  r(A,C)=%.3f  r(B,C)=%.3f\n', ...
            r_AB_obs, r_AC_obs, r_BC_obs);
        fprintf('r(A,C) vs r(A,B):  diff=%.3f  95%%CI=[%.3f, %.3f]  p=%.4f\n', ...
...
function p = compute_2sided_p(diffs)
    ...
    % Two-sided p: proportion of bootstrap samples on the opposite side of zero
