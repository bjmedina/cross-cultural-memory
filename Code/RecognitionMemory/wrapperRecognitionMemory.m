function wrapperRecognitionMemory
    ...
    % this will house "part 1" of the memory experiments. 
    
    rng('shuffle')
    addpath(genpath('~/Tsimane2024/Support/'))
    addpath(genpath('~/Tsimane2024/Data/Fusion/'))
    addpath(genpath('~/Tsimane2024/Code/Fusion/'))

    addpath('~/Tsimane2024/Code/Demography/')
    addpath(genpath('~/Tsimane2024/Stimuli/'))
    LAST_DIRECTORY= '~/Tsimane2024/Data/Last/';  % directory for saving shared state (participant and station)
...
%     end
    ...
    experiment_strings = {"NHS-Globalized", "Industrial-Nature"};

    
    order_dir = '~/Tsimane2024/Data/RecognitionMemory/ExperimentOrders/';
    ...
    if fid~=-1   %if files exist, load them
        ...
        fprintf("Explain the instructions. have translators explain that not all repetitions will happen back to back\n\n ")
...
%         demoExperiment_v1(demo_seq, 1, STATION)
    ...
    for block_number = curr_block_idx:length(block_order)
        ...
        recognitionMemoryMain_v2(subject_name, STATION, seq_for_exp, current_experiment, current_block,coin_flip)
        fprintf('\n\n\n');        
