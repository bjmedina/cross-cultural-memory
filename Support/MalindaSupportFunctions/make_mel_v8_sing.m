function [ s, jitter_matrix ] = make_mel_v8_sing(mel, f0, harm_nums, jitt_amt,jitt, dur_s, sr, dist, centroid, Jitter_Input_Vector)
...
for j=1:length(mel_ratio);
    f = f0*mel_ratio(j);
    [x, jitter_m] = generate_singlenote_vary_envelope_jitter_v2(f, harm_nums, jitt_amt, jitt, dur_s(j), sr, dist, JitterString, centroid);
    s = [s x];
