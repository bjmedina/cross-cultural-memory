function [sh, sw, si_jitt1, si_jitt2, si_sh, sh_shu, previous] = StraightCopy2_expmt_conds_v2_mjm02(x,fs,jitter_amts1,jitter_amts2,shift_factor,harm_shift_up_factor, plot_sg, contour, previous)
...
% Modified 2016-02-28 by Malinda McPherson to add parameters for imposing
% a melody onto the speech. 

% Modified 2016-03-01 by Malinda McPherson to swap speech contours from two utterances 
...
elseif contour ==3 %Impose Melody
    ...
    input_contour = temp3;
elseif contour == 4;
    if length(previous)>=length(sourceStructure.f0)
    ind = length(sourceStructure.f0);
    input_contour = previous(1:ind);
    else 
     ind = length(sourceStructure.f0);
input_contour = [previous; repmat(previous(end), ind-length(previous), 1)];
 input_contour = flipud(input_contour);
    end
    % end
end
