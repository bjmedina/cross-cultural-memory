function outputArgument = deterministicExcitationR4(commandString,currentDataStructure,deterministicHandleOption)
...
switch commandString
    case 'initialize'
        ...
        switch currentDataStructure.sourceType
            ...
            case 'cosUnres' %include only upper harmonics
                lowest_harmonic = biasFactor(1);
                minF0 = min(originalF0(originalF0>32)); 
                maxComponents = floor(fs/2/minF0);
                for ii = lowest_harmonic:maxComponents
                    excitationBuffer = excitationBuffer+cos(integratedPhase*ii).*(f0Interpolated*ii<fs/2);
                end;
            case 'cosRes' %include only lower harmonics
                highest_harmonic = biasFactor(1);
                minF0 = min(originalF0(originalF0>32)); 
                maxComponents = floor(fs/2/minF0);
                for ii = 1:highest_harmonic
                    excitationBuffer = excitationBuffer+cos(integratedPhase*ii).*(f0Interpolated*ii<fs/2);
                end;
        end;
