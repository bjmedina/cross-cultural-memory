function Ps=SYNTH_prepare_voice_bank2_Inharmonic(root_sample_dir,root_seed,IS_PLAY_SOUNDS,fs0,FADE_OUT)
...
for I=1:length(adir)
    ...
    vec=strsplit(vec,'_');
    gender=vec{1};
    assert(strcmp(gender,'fi')||strcmp(gender,'mi'));
    singer_num=vec{2};
    vowel_num=vec{3};
    midi_note=(str2double(vec{4}))/10;
    real_midi_note=(str2double(vec{5}))/10;
    assert(midi_note>30);
    assert(midi_note<110);
    assert(real_midi_note>30);
    assert(real_midi_note<110);
...
for I=1:length(adir)
    ...
    if isfield(INSTRUMENTS{1,ll_idx},'notes')
        INSTRUMENTS{1,ll_idx}.notes(1,end+1)=real_midi_note;
        INSTRUMENTS{1,ll_idx}.patterns{1,end+1}=fname;
    else
        INSTRUMENTS{ll_idx}.notes=real_midi_note;
...
fprintf('Making sound bank for root-dir')
cd (root_sample_dir);
