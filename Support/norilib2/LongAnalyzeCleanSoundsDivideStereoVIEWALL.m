...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [S1,S2]=LongAnalyzeCleanSoundsDivideStereoVIEWALL(myaudio,FS,TRESH,SLEEP,SLEEPMIN,CHUNK,ISPLOT)
...
tt=(1:size(myaudio_norm(1:DELUTE:end,:),1))/FS;
    ...
    if (ISPLOT)
        ...
        plot(tt,-ones(size(myaudio_norm(1:DELUTE:end,1)))*TRESH(1),'g-');hold on;
        plot(tt,myaudio_norm(1:DELUTE:end,1),'Color','b');hold on;
        
        %subplot(2,1,2);
        plot(tt, ones(size(myaudio_norm(1:DELUTE:end,2)))*TRESH(2) +PLOT_SHIFT,'g-');hold on;
        plot(tt,-ones(size(myaudio_norm(1:DELUTE:end,2)))*TRESH(2) +PLOT_SHIFT,'g-');hold on;
        plot(tt,myaudio_norm(1:DELUTE:end,2)+PLOT_SHIFT,'k');hold on;
        set(gca,'Ytick',[-2,0]);
        set(gca,'YtickLabel',{'Response','Stimulus'})
        set(gca,'FontSize',14);
        plot(toplot{1}(:,1),toplot{1}(:,2),'ok','MarkerFaceColor','y');
        plot(toplot{2}(:,1),toplot{2}(:,2)+PLOT_SHIFT,'ok','MarkerFaceColor','y');
