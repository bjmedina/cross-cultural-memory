function M=SYNTH_onsets2Notes(notes,ons,vel,dur,chn)
...
% ons is the onsets of notes, and is suppose to be vector of size of
% notes
...
%
%note!!! ons is in Vorberg format

if length(ons)==1
    ons=cumsum(ones(size(notes))*ons)-ons;
end
% everything is in msec
%size(notes)
%size(ons)
assert(size(notes,2)==size(ons,2));
...
if numel(chn)==1;
    chn=ones(size(ons))*chn;
else
    assert(numel(chn)==numel(ons))
end
