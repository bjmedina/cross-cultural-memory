...
function rec=nori_do_play_record(y,fs0,DEV,verbosity,BEG)
...
for I=1:length(devs)
    ...
    if strcmp(devs(I).DeviceName,DEVi)
        mydev_i=devs(I).DeviceIndex;
        fs_i=devs(I).DefaultSampleRate;
        if verbosity>=1
            fprintf('found input device %s as device index %d\n',devs(I).DeviceName,mydev_i);
        end
        isfound=true;
        break;
    end
...
if ~isfound
    fprintf('did not find the proper device %s ',DEVi);
    assert(1==0);
end
...
for I=1:length(devs)
    ...
    if strcmp(devs(I).DeviceName,DEVo)
        mydev_o=devs(I).DeviceIndex;
        fs_o=devs(I).DefaultSampleRate;
        if verbosity>=1
            fprintf('found output device %s as device index %d\n',devs(I).DeviceName,mydev_o);
        end
        isfound=true;
        break;
    end
...
while (KeyCode~=27) && ((length(recordedaudio) / s.SampleRate) < MAXSECS)

%while (KeyCode(PLOT_SHIFT)~=1) && ((length(recordedaudio) / s.SampleRate) < MAXSECS)
    % Wait a second...
    WaitSecs(1);
    [~, ~, KeyCode]=KbCheck;
