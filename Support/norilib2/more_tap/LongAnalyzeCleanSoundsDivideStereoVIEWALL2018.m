...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [S1,S2]=LongAnalyzeCleanSoundsDivideStereoVIEWALL2018(myaudio,FS,TRESH,SLEEP,SLEEPMIN,CHUNK,ISPLOT)
    ...
    for II=1:length(y(:,1)),
        ...
        if (abs(y(II,2))>TRESH(2))&&(place-last2>mySLEEP(2))
            ...
            if (place-last2>mySLEEPMIN(2))
                ...
                if (ISPLOT)
                    plot(tt(II),PLOT_SHIFT+abs(y(II,2)),'ob','MarkerFaceColor','y');
                    toplot{2}=[toplot{2};tt(II),y(II,2)];
                end
            end
            last2=place;
        end
        ...
        if (abs(y(II,1))>TRESH(1))&&(place-last1>mySLEEP(1))
            ...
            if (place-last1>mySLEEPMIN(1))
                ...
                if (ISPLOT)
                    plot(tt(II),abs(y(II,1)),'ok','MarkerFaceColor','y');
                    toplot{1}=[toplot{1};tt(II),y(II,1)];
                end
            end
...
tt=(1:size(myaudio_norm(1:DELUTE:end,:),1))/FS;
    
    if (ISPLOT)
        %figure(202);clf;subplot(2,1,1);
        hold off;
        plot(tt, ones(size(myaudio_norm(1:DELUTE:end,1)))*TRESH(1),'g-');hold on;
        %plot(tt,-ones(size(myaudio_norm(1:DELUTE:end,1)))*TRESH(1),'g-');hold on;
        plot(tt,abs(myaudio_norm(1:DELUTE:end,1)),'Color','b');hold on;
        
        %subplot(2,1,2);
        plot(tt, ones(size(myaudio_norm(1:DELUTE:end,2)))*TRESH(2) +PLOT_SHIFT,'g-');hold on;
        %plot(tt,-ones(size(myaudio_norm(1:DELUTE:end,2)))*TRESH(2) +PLOT_SHIFT,'g-');hold on;
        plot(tt,abs(myaudio_norm(1:DELUTE:end,2))+PLOT_SHIFT,'r');hold on;
        set(gca,'Ytick',[-2,0]);
        set(gca,'YtickLabel',{'Response','Stimulus'})
        set(gca,'FontSize',14);
        plot(toplot{1}(:,1),abs(toplot{1}(:,2)),'ok','MarkerFaceColor','y');
        plot(toplot{2}(:,1),abs(toplot{2}(:,2))+PLOT_SHIFT,'ob','MarkerFaceColor','y');
        
%         for ll=1:length(oS1)
%             %[~,idx]=min(abs(tt-oS1(ll)/1000));
%             %plot(tt(idx), myaudio_norm(idx),'ok','MarkerFaceColor','y');
%             
%         end
%         for ll=1:length(oS2)
%              [~,idx]=min(abs(tt-oS2(ll)/1000));
%             plot(tt(idx), myaudio_norm(idx)+PLOT_SHIFT,'ok','MarkerFaceColor','y');
%         end
             title(sprintf('sec=%3.3f min= %3.3f',pos/FS,pos/FS/60));
