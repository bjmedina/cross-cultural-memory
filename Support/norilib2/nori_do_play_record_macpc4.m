...
function rec=nori_do_play_record_macpc4(y,fs0,DEV,verbosity,BEG)
...
assert(nrchannels==2);
wavedata=[zeros(2,round(fs*BEG)+1),wavedata,zeros(2,round(fs*BEG)+1)];

if ispc() && (mydev_o==mydev_i)
    % open two channels for input and output
    pout = PsychPortAudio('Open',mydev_o, 3, 0, fs, nrchannels);
    
    %pin= PsychPortAudio('OpenSlave', pout, 2, 0, fs, nrchannels);
    pin=pout;
...
%     MLIM=(MAXSECS-1*BEG);
% else
%     MLIM=(MAXSECS-0.8*BEG);
% end

MLIM=(MAXSECS-1*BEG);
% Stay in a little loop until keypress:
while  ((length(recordedaudio) / s.SampleRate) < MLIM)
    % Wait a second...
    WaitSecs(1);
    [~, ~, KeyCode]=KbCheck;
