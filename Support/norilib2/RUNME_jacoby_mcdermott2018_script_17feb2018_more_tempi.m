...
tans=input('\nSelect tempo (2000 for slow and default value of the experiment, 1000 for fast version of the experiment):');
if isempty(tans)
    tans=2000;
end
...
cd (DATA_DIRECTORY);
DATE_STRING=date();
EXPname=sprintf('EXPiter-%s-%d',DATE_STRING,TOT);
...
switch sans
    case 1 % train
        ...
        REP=10;   % number of repetitions of the cycle (within one iteration)
        GRANDITERs=6; %number of blocks 
    ...
    case 3 %test new (with pause)
        ...
        REP=10;  % number of repetitions of the cycle (within one iteration)
        GRANDITERs=30; %number of blocks
...
for GRANDITER=1:GRANDITERs
    ...
    fprintf('\n\n********************************************************************************\n');
    fprintf('Now in block %d of %d (tempo=%g):\n',GRANDITER,GRANDITERs,TOT);
    Print_stat_participants(SUBJn);
    fprintf('********************************************************************************\n');
